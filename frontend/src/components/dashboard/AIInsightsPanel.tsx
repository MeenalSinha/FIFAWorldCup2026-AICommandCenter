    ...
    <Card className="flex flex-col">
      ...
      {insights.length === 0 ? (
        <p className="px-5 pb-5 text-sm text-[var(--text-secondary)]">
          No active insights right now -- operations are running within normal
          parameters.
        </p>
      ) : (
          ...
          {insights.map((insight) => {
            ...
            return (
              <li key={insight.id}>
                ...
                >
                  <span
                    className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${SEVERITY_BG[insight.severity]}`}
                  >
                    <Icon size={16} />
                  </span>
                  ...
                  <ChevronRight
                    size={16}
                    className="mt-1 text-[var(--text-secondary)] shrink-0"
                  />
                </button>
              </li>
            );
