...
import { TrainFront, Car, ParkingSquare, Footprints, MoreVertical } from "lucide-react";
...
export default function TransportationOverview({ overview }: { overview: TransportationOverviewType["overview"] }) {
  const items = [
    {
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 px-5 pb-5">
        {items.map(({ label, status, detail, icon: Icon, color }) => (
          <div key={label} className="rounded-xl border border-[var(--border-soft)] p-3">
            ...
            <p className={`text-sm font-semibold ${STATUS_COLOR[status] ?? ""}`}>{status}</p>
