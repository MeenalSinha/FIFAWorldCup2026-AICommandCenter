...
export default function VolunteersPage() {
  ...
  return (
    <>
      <Topbar
        title="Volunteer Hub"
        subtitle="AI-generated task prioritization and incident reporting"
      />
      <div className="px-8 pb-10 grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        <Card>
          ...
          {data.volunteers.length === 0 ? (
            <p className="px-5 pb-5 text-sm text-[var(--text-secondary)]">
              No volunteer teams checked in yet.
            </p>
          ) : (
            <ul className="px-5 pb-5 space-y-3">
              {data.volunteers.map((v) => (
                <li
                  key={v.id}
                  className="flex items-center justify-between rounded-xl border border-[var(--border-soft)] px-4 py-3"
                >
                  <div>
        ...
        <Card>
          ...
          {lostFound.items.length === 0 ? (
            <p className="px-5 pb-5 text-sm text-[var(--text-secondary)]">
              No lost & found reports at this time.
            </p>
          ) : (
            <ul className="px-5 pb-5 space-y-3">
